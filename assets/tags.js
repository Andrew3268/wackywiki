/**
 * tags.html 전용 (index와 동일 카드 UI/동작)
 * - /data/posts.json 로드
 * - ?tag=값 으로 1차 필터
 * - 검색(q) + 최신/오래된 정렬 지원
 * - ✅ 더보기(12개 단위) 지원
...
async function loadTagPosts(tagName){
  const slug = slugifyLabel(tagName);
  const url = `${TAG_DIR_URL}${encodeURIComponent(slug)}.json`;
  const data = await fetchJson(url);
  return Array.isArray(data) ? data : [];
}

function setMeta({ total=0, shown=0, tagName="-", sortDesc=true } = {}){
  const countEl = document.getElementById("count");
  const hintEl  = document.getElementById("hint");
  const pillEl  = document.getElementById("tagPill");
  const sortBtn = document.getElementById("sortBtn");

  if (countEl) countEl.textContent = String(shown);
  if (pillEl)  pillEl.textContent  = `태그: ${tagName}`;

  if (hintEl) {
    hintEl.textContent = `총 ${total}개 · ${sortDesc ? "최신순" : "오래된순"} · 태그: ${tagName}`;
  }

  if (sortBtn) {
    // 결과가 2개 이상이면 정렬 버튼 노출
...
function showState(msg){
  ...
  stateEl.innerHTML = `
    ...
    <div>태그 값과 data/posts.json의 tags 배열을 확인해보세요.</div>
  `;
}
...
document.addEventListener("DOMContentLoaded", async () => {  const qEl     = document.getElementById("q");
  ...
  function renderPage(){
    ...
    if(!currentList.length){
      ...
      setMeta({ total:0, shown:0, tagName: tag || "(미지정)", sortDesc });
      return;
    }
    ...
    setMeta({ total: currentList.length, shown: slice.length, tagName: tag, sortDesc });
  }

  if(!tag){
    setMeta({ total:0, shown:0, tagName:"(미지정)", sortDesc:true });
    showState("태그가 지정되지 않았어요. 글에서 태그를 눌러 들어오세요.");
    if(loadMoreWrap) loadMoreWrap.hidden = true;
    return;
  }
